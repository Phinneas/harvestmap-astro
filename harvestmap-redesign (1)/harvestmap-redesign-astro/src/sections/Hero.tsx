...
export default function Hero({ onFind, onUseLocation }: Props) {
  ...
  // Defaults for Astro island usage (no function props across islands):
  // scroll to the finder and, for geolocation, ping the finder's resolver.
  ...
  const handleLocate = () => {
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        () => {
          setLocating(false);
          scrollToFinder(); // fall through to the finder where they can type instead
        },
        { timeout: 6000 }
      );
    } else {
